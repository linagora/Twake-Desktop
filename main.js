...
var template = [{
    ...
    submenu: [
        ...
        { label: "Change server", accelerator: 'CommandOrControl+alt+shift+S', click: function() { changeServer(); } },
        { label: "Open developers tools", accelerator: "CmdOrCtrl+Alt+I", click: function() { win.webContents.openDevTools(); } }
    ]}
];
...
function createWindow(){
    ...
    win.webContents.on('new-window', function(event, url){
      ...
      if(!(url.indexOf("download=1")>0 && url.indexOf(protocol+"://"+domain+"/ajax/")==0)){
        shell.openExternal(url);
      }else{
        download(BrowserWindow.getFocusedWindow(), url, {saveAs: true, showBadge: false})
